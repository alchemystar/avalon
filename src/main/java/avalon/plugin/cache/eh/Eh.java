 ...
 */
package avalon.plugin.cache.eh;
...
public class Eh extends Base {
    ...
    public Eh() {
        if (Eh.cachemanager == null) {
            this.logger.trace("Eh - CacheManager: Loading " + Avalon.config.getProperty("ehcacheConf"));
            Eh.cachemanager = CacheManager.create(Avalon.config.getProperty("ehcacheConf").trim());
        }

        if (Eh.cache == null) {
            this.logger.trace("Eh - cache: Getting " + Avalon.config.getProperty("ehcacheCacheName"));
            Eh.cache = Eh.cachemanager.getEhcache(Avalon.config.getProperty("ehcacheCacheName").trim());
            Eh.cache.setSampledStatisticsEnabled(true);
        }

        if (Eh.cache == null) {
            this.logger.fatal("Eh is null! Does instance '" + Avalon.config.getProperty("ehcacheCacheName") + "' exist?");
        }
    }
