...
import avalon.mysql.proto.Proto;
...
public abstract class MySqlPacket {
    ...
    public static byte[] read_packet(InputStream in) throws IOException {
        ...
        size = MySqlPacket.getSize(packet);
    ...
    public static ArrayList<byte[]> read_full_result_set(InputStream in, OutputStream out, ArrayList<byte[]> buffer,
        ...
        for (int i = 0; i < (colCount + 1); i++) {
            packet = MySqlPacket.read_packet(in);
            if (packet == null) {
                throw new IOException();
            }
            buffer.add(packet);
        ...
        while (true) {
            ...
            int packetType = MySqlPacket.getType(packet);
        ...
        if (MySqlPacket.getType(packet) == Flags.ERR) {
            return buffer;
        }

        if (EOF.loadFromPacket(packet).hasStatusFlag(Flags.SERVER_MORE_RESULTS_EXISTS)) {
            buffer.add(MySqlPacket.read_packet(in));
            buffer = MySqlPacket.read_full_result_set(in, out, buffer, bufferResultSet);
        }
        return buffer;
    }
